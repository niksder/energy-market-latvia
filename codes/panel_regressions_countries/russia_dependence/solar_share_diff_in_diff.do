...
cap mkdir "outputs/panel_countries/solar_diff_and_diff"
...
// Pre-war gas share: value on Feb 23 2022 (fixed treatment intensity per country)
gen _tmp = gas_share if date == td(23feb2021)
bysort country_id: egen gas_share_pre = max(_tmp)
drop _tmp
label var gas_share_pre "Gas share on Feb 23 2022 (pre-war, 0–1)"

// Pre-war solar share: value on Feb 23 2022 (baseline solar penetration per country)
gen _tmp2 = solar_share if date == td(23feb2022)
bysort country_id: egen solar_share_pre = max(_tmp2)
drop _tmp2
...
// Russia dependency (constant per country; country-level)
bysort country_id: egen russia_dep_pre = mean(russia_dependency)
label var russia_dep_pre "Russia gas dependency (constant per country)"
...
di "Pre-war gas share by country:"
table country, statistic(mean gas_share_pre)
di "Pre-war solar share by country:"
table country, statistic(mean solar_share_pre)
di "Pre-war coal share by country:"
table country, statistic(mean coal_share_pre)
di "Treatment (gas_share_pre × russia_dep_pre) by country:"
table country, statistic(mean gas_russia_treat)
...
//
//   α_i  = country fixed effects (absorbed by xtreg fe)
...
//
//   SE clustered at country level (N=14; interpret CI conservatively)
// =============================================================================
...
eststo did_levels
boottest c.gas_russia_treat#1.post, boottype(wild) cluster(country_id) reps(9999) seed(42) // Wild cluster bootstrap for robust inference with few clusters
...
//   β_k > 0 for post-war periods → high-gas countries grew solar more
//
...
// Two-way FE: country absorbed by xtreg fe, period absorbed by ib8.hy_seq_pos.
...
foreach k of local hy_pos_vals {
    if `k' == 8 {
        ...
        quietly boottest inter_hy`k', boottype(wild) cluster(country_id) reps(9999) seed(42) level(95) // Wild cluster bootstrap for robust inference with few clusters
        scalar _es_lb_`i'     = r(CI)[1,1]
        scalar _es_ub_`i'     = r(CI)[1,2]
        quietly boottest inter_hy`k', boottype(wild) cluster(country_id) reps(9999) seed(42) level(90)
...
quietly summarize gas_russia_treat if country == "Latvia"
...
preserve
    ...
    twoway ///
        ...
        note("Two-way FE (country + date). Controls: sun, temperature, ln_precipitation." ///
             "SE clustered at country level (N = 14 countries).", size(vsmall)) ///
        scheme(s2color)

    graph export "outputs/panel_countries/solar_diff_and_diff/event_study_solar_share.png", ///
    ...
    twoway ///
        ...
        note("Each point = estimated extra pp of solar share Latvia gained relative to a country with no pre-war gas." ///
             "Two-way FE (country + date). SE clustered at country level (N = 14 countries).", size(vsmall)) ///
        scheme(s2color)

    graph export "outputs/panel_countries/solar_diff_and_diff/russia_dependence/event_study_latvia_effect_solar_share.png", ///
        replace width(1400) height(900)
restore

di "Done. Outputs saved to outputs/panel_countries/solar_diff_and_diff/russia_dependence/"
