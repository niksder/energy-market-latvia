...
//   β_k > 0 for post-war periods → high-fossil bzones faced higher prices
//
//   Period mapping:
...
preserve
    ...
    twoway ///
        ...
        ytitle("Coef (log price per pp of pre-war fossil share)") ///
        title("Effect of Pre-War Fossil Exposure on Electricity Price") ///
    ...
    graph export "outputs/panel/electricity/solanka/event_study_elect_price_on_fossil.png", ///
...
file open `fh_tex' using ///
    "outputs/panel/electricity/solanka/event_study_elect_price_on_fossil.tex", ///
    write replace

file write `fh_tex' "\begin{table}[htbp]" _n
file write `fh_tex' "\centering" _n
file write `fh_tex' "\caption{Event study: effect of pre-war fossil exposure on log electricity price}" _n
file write `fh_tex' "\label{tab:event_study_price_fossil}" _n
...
di "LaTeX table saved to outputs/panel/electricity/solanka/event_study_elect_price_on_fossil.tex"
