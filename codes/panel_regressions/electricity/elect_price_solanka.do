...
cap mkdir "outputs/panel/electricity/solanka"
...
//   β_k > 0 for post-war periods → high-gas bzones faced higher prices
//
//   Period mapping:
...
xtreg ln_energy_price `inter_vars' ///
...
preserve
    ...
    twoway ///
        ...
        ytitle("Coef (log price per pp of pre-war gas share)") ///
        title("Effect of Pre-War Gas Exposure on Electricity Price") ///
    ...
    graph export "outputs/panel/electricity/solanka/event_study_elect_price.png", ///
...
file open `fh_tex' using ///
    "outputs/panel/electricity/solanka/event_study_elect_price.tex", ///
    write replace

file write `fh_tex' "\begin{table}[htbp]" _n
file write `fh_tex' "\centering" _n
file write `fh_tex' "\caption{Event study: effect of pre-war gas exposure on log electricity price}" _n
file write `fh_tex' "\label{tab:event_study_price_gas}" _n
file write `fh_tex' "\begin{tabular}{lc}" _n
file write `fh_tex' "\hline\hline" _n
file write `fh_tex' "Period & Log electricity price \\" _n
...
file write `fh_tex' " Dependent variable: log electricity price." _n
...
di "LaTeX table saved to outputs/panel/electricity/solanka/event_study_elect_price.tex"
di "Done. Outputs saved to outputs/panel/electricity/solanka/"
