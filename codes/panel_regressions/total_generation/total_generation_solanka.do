...
cap mkdir "outputs/panel/total_generation"
cap mkdir "outputs/panel/total_generation/solanka"

// drop "IT_SACOAC" bzone == "IT_SACODC" 'IT_CALA', 'SE1', 'SE2', 'SE3', 'SE4', 'Cyprus', 'Germany', 'Croatia'
...
// =============================================================================
// TREATMENT VARIABLE: pre-war gas share (Feb 23 2021, last full year before war)
// =============================================================================
...
// =============================================================================
// EVENT STUDY
//   Y_it = α_i + γ_t + Σ_k β_k*(gas_share_pre_i × 1[hy_seq_pos=k]) + controls + ε_it
//
//   Reference period: H2 2020 (hy_seq_pos = 8)
//   β_k ≈ 0 for pre-war periods → parallel trends
//   β_k ≠ 0 for post-war periods → high-gas bzones changed total generation differently
//
//   Period mapping:
...
// =============================================================================
// NORMALIZE total_generation to the reference period (H2 2020) mean per bzone
//   total_generation_norm = 1 in H2 2020; values in other periods reflect
//   relative change, making bzones comparable regardless of scale
// =============================================================================

gen _tmp_ref = total_generation_yearly if hy_seq_pos == 8
bysort bzone_id: egen total_gen_ref = mean(_tmp_ref)
drop _tmp_ref
gen total_generation_norm = total_generation_yearly / total_gen_ref
label var total_generation_norm "Total generation yearly (normalized: H2 2020 = 1)"
...
xtreg total_generation_norm `inter_vars' ///
...
preserve
    ...
    twoway ///
        ...
        ytitle("Coef (normalized total generation per pp of pre-war gas share)") ///
        title("Effect of Pre-War Gas Exposure on Total Generation") ///
    ...
    graph export "outputs/panel/total_generation/solanka/event_study_total_generation.png", ///
...
file open `fh_tex' using ///
    "outputs/panel/total_generation/solanka/event_study_total_generation.tex", ///
    write replace

file write `fh_tex' "\begin{table}[htbp]" _n
file write `fh_tex' "\centering" _n
file write `fh_tex' "\caption{Event study: effect of pre-war gas exposure on total generation}" _n
file write `fh_tex' "\label{tab:event_study_total_gen_gas}" _n
file write `fh_tex' "\begin{tabular}{lc}" _n
file write `fh_tex' "\hline\hline" _n
file write `fh_tex' "Period & Total generation (normalized) \\" _n
...
file write `fh_tex' " Dependent variable: total generation normalized to H2~2020 mean per bidding zone." _n
...
di "LaTeX table saved to outputs/panel/total_generation/solanka/event_study_total_generation.tex"
di "Done. Outputs saved to outputs/panel/total_generation/solanka/"
