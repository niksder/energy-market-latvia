...
# Download energy sources from ENTSOE API and save to CSV file

def download_energy_sources(year=2026, energy_type=None):
    ...
    if response.status_code == 200:
        ...
        file_path = os.path.join(DATA_DIR, ENERGY_SOURCES_DIR, f'energy_sources_{year}_{energy_type}.csv')
        with open(file_path, 'wb') as file:
            file.write(response.content)
        print(f'Energy sources for {year} and energy type {energy_type} downloaded successfully and saved to {file_path}')
    else:
        print(f'Failed to download energy sources for {year} and energy type {energy_type}. Status code: {response.status_code}')
...
for year in years:
    for energy_type in energy_types_included:
        ...
        download_energy_sources(year, energy_type)
