...
os.makedirs(os.path.join(DATA_DIR, ENERGY_PRICES_EUROPE_DIR), exist_ok=True)

# Download energy prices from ENTSOE API and save to CSV file

def download_energy_prices(country, year=2026):
    # Define the API endpoint and parameters
    api_url = 'https://web-api.tp.entsoe.eu/api'
    params = {
        'documentType': 'A44',
        'out_Domain': country['code'],
        'in_Domain': country['code'],
    ...
    if response.status_code == 200:
        ...
        print(f'Energy prices for {country["name"]} {year} downloaded successfully and saved to {file_path}')
    else:
        print(f'Failed to download energy prices for {country["name"]} {year}. Status code: {response.status_code}')
...
for country in COUNTRIES:
    for year in years:
        ...
        download_energy_prices(country, year)