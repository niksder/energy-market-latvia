...
PLOT_OUTPUT_PATH = OUTPUTS_DIR + "/panel_countries/others/gas_share_of_countries.png"
...
# --- Load data ---
df = pd.read_csv(MERGED_PANEL_DATA_PATH, usecols=['country', 'year', 'gas_production', 'total_generation'])
df = df[(df['year'] >= START_DATE.year) & (df['year'] <= END_DATE.year)]

# --- Compute annual gas share per country ---
annual = (
    df.groupby(['country', 'year'])[['gas_production', 'total_generation']]
    .sum()
    .reset_index()
)
...
table = annual.pivot(index='country', columns='year', values='gas_share')
