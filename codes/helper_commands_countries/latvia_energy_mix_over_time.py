...
PLOT_OUTPUT_PATH = OUTPUTS_DIR + "/panel_countries/others/latvia_energy_mix_over_time.png"
START_DATE = pd.Timestamp("2017-01-01")
END_DATE = pd.Timestamp("2025-12-31")

# --- Load data ---
df = pd.read_csv(MERGED_PANEL_DATA_PATH, usecols=['country', 'year', 'gas_production', 'solar_production', 'total_generation'])
df = df[(df['year'] >= START_DATE.year) & (df['year'] <= END_DATE.year)]

# --- Filter Latvia only ---
lv = df[df['country'] == 'Latvia']
