...
def process_merged_data_countries():
    # Reads the country panel CSV in chunks (one country at a time) so peak
    # memory is proportional to the largest single country rather than the full file.
    # Writes to a temp file and atomically replaces COUNTRY_PANEL_DATA_PATH at the end,
    ...
    def _flush(country_name, rows):
        ...
        is_latvia_group = group['country'] == 'Latvia'
        ...
        write_header = False
        n_written += 1
        print(f'  [{n_written}] wrote {country_name}')
        del group
        gc.collect()

    try:
        for chunk in pd.read_csv(COUNTRY_PANEL_DATA_PATH, chunksize=50_000, dtype={'country': str}):
            ...
            # File is sorted by country; detect country boundaries and flush completed countries.
            for country in chunk['country'].unique():
                c_rows = chunk[chunk['country'] == country].copy()
    ...
    print(f'Added rolling shares/yearly/growth for: {prefixes_added}; accumulated precipitation; policy columns. Wrote to {COUNTRY_PANEL_DATA_PATH}')
    gc.collect()

if __name__ == '__main__':
    process_merged_data_countries()
